...
def storage(number, title, author, timeout, text, label, statement):
    dicts = {
        "_id": number,
        "title": title,
    ...
    storageDatabase(dicts, come_from="hangliancj_alerts")

def download(url, reponse, number):
    try:
        print("hangliancj_alerts")
        ...
        storage(number, title, author, timeout, text, label, statement)
