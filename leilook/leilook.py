...
from leilook import leilook_alerts
from mongodb_news import storageDatabase, rechecking


def storage(number, title, author, timeout, text, label, img):
    dicts = {
        "_id":number,
        "title": title,
        "author": author + "--快讯",
        "release_time": "2018年" + timeout[0],
        "main": text,
    ...
    # print(dicts)
    storageDatabase(dicts, come_from="leilook")
...
def download(html, number, url):
    ...
    try:
        ...
        if len(texts) > 2:
            ...
            label = etree.tostring(label, method="text", encoding="utf8").decode("utf8").split()
            if label:
                label = label[0]
            else:
                label = ""
            ...
            img = img_p + img_div_p
            storage(number, title, author, timeout, text, label, img)
        else:
            leilook_alerts.download(html, number, url)
