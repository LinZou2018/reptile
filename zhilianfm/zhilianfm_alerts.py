...
def storage(number, title, author, timeout, source, text, classify):
    dicts = {
        "_id": number,
        "title": title,
        "author": author,
    ...
    # print(dicts)
    storageDatabase(dicts, come_from="zhilianfm_alerts")
...
def download(number):
    # try:
        ...
        if reponse.status_code == 200:
            ...
            # 获取文章内容
            texts = html.xpath('/html/body/div[2]/section/div[2]/text()')[0].split()
            text = ""
            for i in texts:
                text += i + " "
